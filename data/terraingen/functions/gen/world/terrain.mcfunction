...
#sea level
scoreboard players operation @s seed = sealevel seed

#noise
function #terraingen:noise
summon area_effect_cloud ~ ~ ~ {Duration:100,Tags:["PlaceBlock"]} 
execute store result entity @e[tag=PlaceBlock,limit=1] Pos[1] double 0.002 run scoreboard players operation @s seed += @s perlinOutput
